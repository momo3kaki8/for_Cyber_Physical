import requests
from bs4 import BeautifulSoup
import pandas as pd
import time
import os

def get_data_by_requests(url):
    print(f"Fetching: {url}")
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
    }
    
    try:
        res = requests.get(url, headers=headers, timeout=10)
        res.encoding = res.apparent_encoding
        soup = BeautifulSoup(res.text, 'html.parser')
        
        data = {"URL": url}
        
        # 1. 商品名の取得
        title_tag = soup.find('h2', class_='ttl') or soup.find('h1')
        data["商品名"] = title_tag.get_text(strip=True) if title_tag else "商品名不明"

        # 2. 画像URLの取得 (imgタグのsrcを取得)
        # 商品画像は通常、特定のクラス内や、特定のパス（/recommend/original/detail/img/）を含みます
        img_tag = soup.find('img', src=lambda s: s and '/recommend/original/detail/img/' in s)
        if img_tag:
            img_src = img_tag.get('src')
            # 相対パスを絶対URLに変換
            data["画像URL"] = "https://www.lawson.co.jp" + img_src
        else:
            data["画像URL"] = "画像なし"

        # 3. 価格の取得
        price_tag = soup.find('dl', class_='price')
        if price_tag:
            data["価格"] = price_tag.get_text(strip=True).replace('ローソン標準価格', '')

        # 4. 栄養成分の抽出
        nutrition_div = soup.find('div', class_='nutritionFacts_table')
        if nutrition_div:
            for dl in nutrition_div.find_all('dl'):
                dt, dd = dl.find('dt'), dl.find('dd')
                if dt and dd:
                    data[dt.get_text(strip=True)] = dd.get_text(strip=True)
        
        return data

    except Exception as e:
        print(f"Error: {e}")
        return None

def main():
    if not os.path.exists("urls_rice.txt"):
        print("エラー: urls.txt が見つかりません。")
        return

    with open("urls_rice.txt", "r", encoding="utf-8") as f:
        urls = [line.strip() for line in f if line.strip()]

    all_data = []
    for i, url in enumerate(urls):
        print(f"[{i+1}/{len(urls)}]", end=" ")
        result = get_data_by_requests(url)
        if result:
            all_data.append(result)
        time.sleep(1)

    if all_data:
        df = pd.DataFrame(all_data)
        # 列の順番を整理（画像URLを価格の次あたりに配置）
        first_cols = ['商品名', '価格', '画像URL', 'URL']
        other_cols = [c for c in df.columns if c not in first_cols]
        existing_first_cols = [c for c in first_cols if c in df.columns]
        df = df[existing_first_cols + other_cols]
        
        df.to_csv("lawson_nutrition_with_img.csv", index=False, encoding="utf-8-sig")
        print(f"\n完了！ 画像URLを含めたデータを lawson_nutrition_with_img.csv に保存しました。")
    else:
        print("データが取得できませんでした。")

if __name__ == "__main__":
    main()